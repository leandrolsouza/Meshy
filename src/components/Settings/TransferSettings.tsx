...
export function validateTransferFields(
    ...
    const speedLimitMsg = intl
        ? intl.formatMessage({ id: 'settings.transfer.invalidSpeedLimit' })
        : 'Valor inválido: deve ser um inteiro não-negativo.';
    ...
    if (!isValidSpeedLimit(Number(downloadLimit))) {
        setDownloadLimitError(speedLimitMsg);
        hasError = true;
    }

    if (!isValidSpeedLimit(Number(uploadLimit))) {
        setUploadLimitError(speedLimitMsg);
        hasError = true;
    }

    if (!isValidMaxConcurrentDownloads(Number(maxConcurrent))) {
        setMaxConcurrentError(maxConcurrentMsg);
        hasError = true;
    }
        ...
        <>
            ...
            <div className={styles.fieldGroup}>
                <label htmlFor="download-speed-limit" className="label">
                    {intl.formatMessage({ id: 'settings.transfer.downloadLimit' })}
                </label>
                <input
            ...
            <div className={styles.fieldGroup}>
                <label htmlFor="upload-speed-limit" className="label">
                    {intl.formatMessage({ id: 'settings.transfer.uploadLimit' })}
                </label>
                <input
            ...
            <div className={styles.fieldGroup}>
                <label htmlFor="max-concurrent-downloads" className="label">
                    {intl.formatMessage({ id: 'settings.transfer.maxConcurrent' })}
                </label>
                <input
