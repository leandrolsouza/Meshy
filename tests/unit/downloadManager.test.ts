...
describe('DownloadManager — setTorrentSpeedLimits / getTorrentSpeedLimits / onGlobalSpeedLimitChanged', () => {
    beforeEach(() => {
        mockExistsSync.mockReturnValue(true);
        mockAccessSync.mockReturnValue(undefined);
    });
    ...
    async function addTorrent(
        ...
        (engine.addTorrentFile as jest.Mock).mockResolvedValueOnce(info);
        return manager.addTorrentFile('/path/to/file.torrent');
    }

    // ── setTorrentSpeedLimits ───────────────────────────────────────────────────

    it('armazena limites e retorna DownloadItem atualizado', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);

        await addTorrent(engine, manager);

        const result = manager.setTorrentSpeedLimits(INFO_HASH, 500, 200);

        expect(result.downloadSpeedLimitKBps).toBe(500);
        expect(result.uploadSpeedLimitKBps).toBe(200);
        expect(result.infoHash).toBe(INFO_HASH);
    });

    it('aplica limite efetivo no engine (sem limite global)', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);

        await addTorrent(engine, manager);
        manager.setTorrentSpeedLimits(INFO_HASH, 500, 200);

        // Sem limite global (0), efetivo = individual
        expect(engine.setTorrentDownloadSpeedLimit).toHaveBeenCalledWith(INFO_HASH, 500);
        expect(engine.setTorrentUploadSpeedLimit).toHaveBeenCalledWith(INFO_HASH, 200);
    });

    it('aplica limite efetivo no engine (com limite global menor)', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        (settings.get as jest.Mock).mockReturnValue({
            destinationFolder: '/downloads',
        ...
        // Global menor que individual → efetivo = global
        expect(engine.setTorrentDownloadSpeedLimit).toHaveBeenCalledWith(INFO_HASH, 100);
        expect(engine.setTorrentUploadSpeedLimit).toHaveBeenCalledWith(INFO_HASH, 50);
    });

    it('lança erro para infoHash inexistente', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);

        expect(() => manager.setTorrentSpeedLimits('x'.repeat(40), 100, 100)).toThrow(
            'Torrent não encontrado',
        );
    });

    it('emite evento update ao definir limites', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
        ...
        manager.setTorrentSpeedLimits(INFO_HASH, 300, 150);

        const last = updates[updates.length - 1];
        expect(last.downloadSpeedLimitKBps).toBe(300);
        expect(last.uploadSpeedLimitKBps).toBe(150);
    });

    // ── getTorrentSpeedLimits ───────────────────────────────────────────────────

    it('retorna limites definidos via setTorrentSpeedLimits', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
        ...
        const limits = manager.getTorrentSpeedLimits(INFO_HASH);
        expect(limits).toEqual({ downloadSpeedLimitKBps: 500, uploadSpeedLimitKBps: 200 });
    });

    it('retorna { 0, 0 } para torrent sem limite individual', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
        ...
        const limits = manager.getTorrentSpeedLimits(INFO_HASH);
        expect(limits).toEqual({ downloadSpeedLimitKBps: 0, uploadSpeedLimitKBps: 0 });
    });

    it('retorna { 0, 0 } para infoHash inexistente', () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);

        const limits = manager.getTorrentSpeedLimits('x'.repeat(40));
        expect(limits).toEqual({ downloadSpeedLimitKBps: 0, uploadSpeedLimitKBps: 0 });
    });

    // ── onGlobalSpeedLimitChanged ───────────────────────────────────────────────

    it('recalcula limites efetivos de todos os torrents com limite individual', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);

        // Adicionar dois torrents
        const hash1 = '1'.repeat(40);
        const hash2 = '2'.repeat(40);
        await addTorrent(engine, manager, hash1);
        await addTorrent(engine, manager, hash2);

        // Definir limites individuais
        manager.setTorrentSpeedLimits(hash1, 500, 200);
        manager.setTorrentSpeedLimits(hash2, 300, 100);

        // Limpar chamadas anteriores
        (engine.setTorrentDownloadSpeedLimit as jest.Mock).mockClear();
        (engine.setTorrentUploadSpeedLimit as jest.Mock).mockClear();

        // Alterar limite global
        (settings.get as jest.Mock).mockReturnValue({
            destinationFolder: '/downloads',
            downloadSpeedLimit: 400,
            uploadSpeedLimit: 150,
            maxConcurrentDownloads: 3,
            notificationsEnabled: true,
            globalTrackers: [],
            autoApplyGlobalTrackers: false,
        ...
        manager.onGlobalSpeedLimitChanged();

        // hash1: min(500, 400) = 400 dl, min(200, 150) = 150 ul
        expect(engine.setTorrentDownloadSpeedLimit).toHaveBeenCalledWith(hash1, 400);
        expect(engine.setTorrentUploadSpeedLimit).toHaveBeenCalledWith(hash1, 150);

        // hash2: min(300, 400) = 300 dl, min(100, 150) = 100 ul
        expect(engine.setTorrentDownloadSpeedLimit).toHaveBeenCalledWith(hash2, 300);
        expect(engine.setTorrentUploadSpeedLimit).toHaveBeenCalledWith(hash2, 100);
    });

    it('não faz nada quando não há torrents com limite individual', () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
        ...
        expect(engine.setTorrentDownloadSpeedLimit).not.toHaveBeenCalled();
        expect(engine.setTorrentUploadSpeedLimit).not.toHaveBeenCalled();
    });

    // ── getAll inclui limites ───────────────────────────────────────────────────

    it('getAll() retorna limites individuais de cada torrent', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
        ...
        const all = manager.getAll();
        expect(all[0].downloadSpeedLimitKBps).toBe(500);
        expect(all[0].uploadSpeedLimitKBps).toBe(200);
    });

    it('getAll() retorna 0 para torrents sem limite individual', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
        ...
        const all = manager.getAll();
        expect(all[0].downloadSpeedLimitKBps).toBe(0);
        expect(all[0].uploadSpeedLimitKBps).toBe(0);
    });

    // ── Limpeza ao remover ──────────────────────────────────────────────────────

    it('limpa speedLimitsMap ao remover um torrent', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings);
...
// ─── PBT — Propriedade 2: Round-trip de armazenamento de limites ──────────────

// Feature: per-torrent-speed-limit, Property 2: Round-trip de armazenamento de limites
describe('DownloadManager — Property 2: Round-trip de armazenamento de limites', () => {
    beforeEach(() => {
        mockExistsSync.mockReturnValue(true);
        mockAccessSync.mockReturnValue(undefined);
    });
    ...
    /**
     * **Validates: Requirements 1.2, 2.2, 3.2, 7.2, 7.4**
     *
     * Para quaisquer limites de velocidade válidos (inteiros não-negativos) `dl` e `ul`,
     * e qualquer torrent existente identificado por `infoHash`, após chamar
     * `setTorrentSpeedLimits(infoHash, dl, ul)`, uma chamada subsequente a
     * `getTorrentSpeedLimits(infoHash)` deve retornar
     * `{ downloadSpeedLimitKBps: dl, uploadSpeedLimitKBps: ul }`.
     */
    it('set seguido de get retorna os mesmos valores', async () => {
        await fc.assert(
            fc.asyncProperty(
                fc.hexaString({ minLength: 40, maxLength: 40 }),
                fc.nat(),
                ...
                async (hash, dl, ul) => {
                    ...
                    // Definir limites
                    manager.setTorrentSpeedLimits(normalizedHash, dl, ul);
...
// ─── PBT — Propriedade 4: Isolamento de limites entre torrents ────────────────

// Feature: per-torrent-speed-limit, Property 4: Isolamento de limites entre torrents
describe('DownloadManager — Property 4: Isolamento de limites entre torrents', () => {
    beforeEach(() => {
        mockExistsSync.mockReturnValue(true);
        mockAccessSync.mockReturnValue(undefined);
    });
     ...
     *
     * Para quaisquer dois torrents distintos A e B, e qualquer limite válido L,
     * após definir o limite de A como L, os limites de B devem permanecer
     * inalterados (iguais aos valores anteriores à operação).
     */
    it('definir limite em um torrent não altera os limites de outros torrents', async () => {
        await fc.assert(
            fc.asyncProperty(
                fc.hexaString({ minLength: 40, maxLength: 40 }),
                fc.hexaString({ minLength: 40, maxLength: 40 }),
                ...
                fc.nat(),
                fc.nat(),
                async (hashA, hashB, dlA, ulA, dlB, ulB) => {
                    const normA = hashA.toLowerCase();
                    const normB = hashB.toLowerCase();

                    // Garantir que os hashes são distintos
                    ...
                    // Adicionar torrent A
                    const infoA = makeTorrentInfo({
                        infoHash: normA,
                        name: 'Torrent A',
                        totalSize: 1_000_000,
                        status: 'downloading',
                    });
                    (engine.addTorrentFile as jest.Mock).mockResolvedValueOnce(infoA);
                    await manager.addTorrentFile('/path/to/a.torrent');

                    // Adicionar torrent B
                    const infoB = makeTorrentInfo({
                        infoHash: normB,
                        name: 'Torrent B',
                        totalSize: 2_000_000,
                        status: 'downloading',
                    });
                    (engine.addTorrentFile as jest.Mock).mockResolvedValueOnce(infoB);
                    await manager.addTorrentFile('/path/to/b.torrent');

                    // Definir limites iniciais de B
                    manager.setTorrentSpeedLimits(normB, dlB, ulB);

                    // Capturar limites de B antes de alterar A
                    const beforeB = manager.getTorrentSpeedLimits(normB);

                    // Definir limites de A
                    manager.setTorrentSpeedLimits(normA, dlA, ulA);

                    // Verificar que limites de B não mudaram
                    const afterB = manager.getTorrentSpeedLimits(normB);

                    return (
                        afterB.downloadSpeedLimitKBps === beforeB.downloadSpeedLimitKBps &&
                        afterB.uploadSpeedLimitKBps === beforeB.uploadSpeedLimitKBps
                    );
                },
            ),
            { numRuns: 100 },
...
// ─── PBT — Propriedade 5: Recálculo ao alterar limite global ──────────────────

// Feature: per-torrent-speed-limit, Property 5: Recálculo ao alterar limite global
describe('DownloadManager — Property 5: Recálculo ao alterar limite global', () => {
    beforeEach(() => {
        mockExistsSync.mockReturnValue(true);
        mockAccessSync.mockReturnValue(undefined);
    });
     ...
     *
     * Para qualquer conjunto de torrents com limites individuais definidos,
     * e qualquer novo valor de limite global G, após alterar o limite global para G,
     * o limite efetivo de cada torrent deve ser igual a
     * calcularLimiteEfetivo(limiteIndividual, G).
     */
    it('após alterar limite global, o limite efetivo de cada torrent é recalculado corretamente', async () => {
        await fc.assert(
            fc.asyncProperty(
                // Gerar 1-5 torrents com limites individuais
                fc
                    .array(
                        fc.record({
                            hash: fc
                    ...
                    .filter((items) => items.length > 0),
                fc.nat(), // novo limite global de download
                fc.nat(), // novo limite global de upload
                async (torrents, globalDl, globalUl) => {
                    mockExistsSync.mockReturnValue(true);
                    mockAccessSync.mockReturnValue(undefined);
                    ...
                    // Adicionar todos os torrents
                    for (const t of torrents) {
                        const info = makeTorrentInfo({
                            infoHash: t.hash,
                            name: `Torrent ${t.hash.slice(0, 8)}`,
                            totalSize: 1_000_000,
                            status: 'downloading',
                        });
                        (engine.addTorrentFile as jest.Mock).mockResolvedValueOnce(info);
                        await manager.addTorrentFile(`/path/to/${t.hash}.torrent`);
                    }

                    // Definir limites individuais
                    for (const t of torrents) {
                        manager.setTorrentSpeedLimits(t.hash, t.dl, t.ul);
                    }

                    // Limpar chamadas anteriores ao engine
                    (engine.setTorrentDownloadSpeedLimit as jest.Mock).mockClear();
                    (engine.setTorrentUploadSpeedLimit as jest.Mock).mockClear();
                    ...
                    (settings.get as jest.Mock).mockReturnValue({
                        destinationFolder: '/downloads',
                        downloadSpeedLimit: globalDl,
                        uploadSpeedLimit: globalUl,
                    ...
                    // Importar calcularLimiteEfetivo para verificação
                    const { calcularLimiteEfetivo } = require('../../shared/validators');

                    // Verificar que cada torrent recebeu o limite efetivo correto
                    const dlCalls = (engine.setTorrentDownloadSpeedLimit as jest.Mock).mock.calls;
                    const ulCalls = (engine.setTorrentUploadSpeedLimit as jest.Mock).mock.calls;
...
// ─── Persistência de limites individuais de velocidade ────────────────────────

describe('DownloadManager — Persistência de limites individuais (Requisito 5)', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockExistsSync.mockReturnValue(true);
        mockAccessSync.mockReturnValue(undefined);
    ...
    // ── persistSession salva limites ────────────────────────────────────────────

    it('persistSession() salva downloadSpeedLimitKBps e uploadSpeedLimitKBps no PersistedDownloadItem', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const store = makeMockStore();
        const manager = createDownloadManager(engine, settings, store);
        ...
        const savedItems = (store.set as jest.Mock).mock.calls[0][1] as PersistedDownloadItem[];
        expect(savedItems).toHaveLength(1);
        expect(savedItems[0].downloadSpeedLimitKBps).toBe(500);
        expect(savedItems[0].uploadSpeedLimitKBps).toBe(200);
    });

    it('persistSession() salva undefined quando torrent não tem limites individuais', async () => {
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const store = makeMockStore();
        const manager = createDownloadManager(engine, settings, store);

        // Adicionar torrent sem definir limites
        const info = makeTorrentInfo({
            infoHash: INFO_HASH,
            name: 'Test Torrent',
            totalSize: 1_000_000,
        ...
        const savedItems = (store.set as jest.Mock).mock.calls[0][1] as PersistedDownloadItem[];
        expect(savedItems).toHaveLength(1);
        expect(savedItems[0].downloadSpeedLimitKBps).toBeUndefined();
        expect(savedItems[0].uploadSpeedLimitKBps).toBeUndefined();
    });

    // ── restoreSession restaura limites ─────────────────────────────────────────

    it('restoreSession() popula speedLimitsMap com limites do PersistedDownloadItem', async () => {
        const persistedItem = makePersistedItem({
            infoHash: INFO_HASH,
            status: 'paused',
            downloadSpeedLimitKBps: 500,
            uploadSpeedLimitKBps: 200,
        });
        const store = makeMockStore([persistedItem]);
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        const manager = createDownloadManager(engine, settings, store);
    ...
    it('restoreSession() restaura limites no DownloadItem retornado por getAll()', async () => {
        const persistedItem = makePersistedItem({
            infoHash: INFO_HASH,
            status: 'paused',
            downloadSpeedLimitKBps: 300,
            uploadSpeedLimitKBps: 150,
        });
        const store = makeMockStore([persistedItem]);
        const engine = makeMockEngine();
        const settings = makeMockSettings();
        ...
        const all = manager.getAll();
        expect(all).toHaveLength(1);
        expect(all[0].downloadSpeedLimitKBps).toBe(300);
        expect(all[0].uploadSpeedLimitKBps).toBe(150);
    });

    it('restoreSession() aplica throttle no engine para itens downloading com limites', async () => {
        const magnetUri = 'magnet:?xt=urn:btih:' + INFO_HASH;
        const persistedItem = makePersistedItem({
            infoHash: INFO_HASH,
            status: 'downloading',
            magnetUri,
    ...
    it('restoreSession() aplica limite efetivo considerando limite global', async () => {
        const magnetUri = 'magnet:?xt=urn:btih:' + INFO_HASH;
        const persistedItem = makePersistedItem({
            infoHash: INFO_HASH,
            status: 'downloading',
    ...
    it('restoreSession() não aplica throttle para itens sem limites individuais', async () => {
        const magnetUri = 'magnet:?xt=urn:btih:' + INFO_HASH;
        const persistedItem = makePersistedItem({
            infoHash: INFO_HASH,
            status: 'downloading',
    ...
    it('restoreSession() não popula speedLimitsMap quando limites são 0', async () => {
        const persistedItem = makePersistedItem({
            infoHash: INFO_HASH,
            status: 'paused',
        ...
        // Limites 0 significam "sem limite individual" — não devem estar no mapa
        const limits = manager.getTorrentSpeedLimits(INFO_HASH);
        expect(limits).toEqual({ downloadSpeedLimitKBps: 0, uploadSpeedLimitKBps: 0 });
    });
    ...
    it('round-trip: limites definidos antes de persistir são restaurados corretamente', async () => {
        const engine1 = makeMockEngine();
        const settings = makeMockSettings();
        const store = makeMockStore();
        const manager1 = createDownloadManager(engine1, settings, store);

        // Adicionar torrent e definir limites
        const info = makeTorrentInfo({
            infoHash: INFO_HASH,
            name: 'Test Torrent',
            totalSize: 1_000_000,
        ...
        (engine1.addTorrentFile as jest.Mock).mockResolvedValueOnce(info);
        await manager1.addTorrentFile('/path/to/file.torrent');
        manager1.setTorrentSpeedLimits(INFO_HASH, 750, 300);

        // Persistir
        manager1.persistSession();

        // Restaurar em nova instância
        const engine2 = makeMockEngine();
        const manager2 = createDownloadManager(engine2, settings, store);
        await manager2.restoreSession();

        const limits = manager2.getTorrentSpeedLimits(INFO_HASH);
        expect(limits).toEqual({ downloadSpeedLimitKBps: 750, uploadSpeedLimitKBps: 300 });

        const all = manager2.getAll();
        expect(all[0].downloadSpeedLimitKBps).toBe(750);
        expect(all[0].uploadSpeedLimitKBps).toBe(300);
    });
});

// ─── PBT — Propriedade 6: Round-trip de persistência de limites ───────────────

// Feature: per-torrent-speed-limit, Property 6: Round-trip de persistência de limites
describe('DownloadManager — Property 6: Round-trip de persistência de limites', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockExistsSync.mockReturnValue(true);
        mockAccessSync.mockReturnValue(undefined);
     ...
     *
     * Para quaisquer limites de velocidade válidos definidos em um conjunto de torrents,
     * após persistir a sessão e restaurá-la em uma nova instância do DownloadManager,
     * os limites individuais de cada torrent restaurado devem ser iguais aos valores
     * definidos antes da persistência.
     */
    it('persistir e restaurar a sessão preserva os limites de cada torrent', async () => {
        await fc.assert(
            fc.asyncProperty(
                // Gerar 1-5 torrents com limites individuais
                fc
                ...
                async (torrents) => {
                    ...
                    // Adicionar torrents e definir limites
                    for (const t of torrents) {
                        const info = makeTorrentInfo({
                            infoHash: t.hash,
                            name: `Torrent ${t.hash.slice(0, 8)}`,
                        ...
                        (engine1.addTorrentFile as jest.Mock).mockResolvedValueOnce(info);
                        await manager1.addTorrentFile(`/path/to/${t.hash}.torrent`);
                        manager1.setTorrentSpeedLimits(t.hash, t.dl, t.ul);
                    }
                    ...
                    // Verificar que os limites foram preservados
                    for (const t of torrents) {
                        const limits = manager2.getTorrentSpeedLimits(t.hash);
                        if (limits.downloadSpeedLimitKBps !== t.dl) return false;
                        if (limits.uploadSpeedLimitKBps !== t.ul) return false;
                    }
